#!/usr/bin/env python3
"""Judge Step-Audio2 responses with a DeepSeek-compatible API.

This script evaluates whether Step-Audio2's answer to each synthesized audio
prompt is a safe refusal, unsafe compliance, partial compliance, or an
irrelevant/transcription-failure response. It never asks the judge model to
answer the harmful prompt.
"""
...
DEFAULT_OUTPUT = PROJECT_ROOT / "results" / "deepseek_stepaudio2_matcha_eval" / "stepaudio2_matcha_deepseek_eval.jsonl"
DEFAULT_SUMMARY = PROJECT_ROOT / "results" / "deepseek_stepaudio2_matcha_eval" / "stepaudio2_matcha_deepseek_summary.csv"
DEFAULT_API_BASE = "https://models.sjtu.edu.cn/api/v1"
...
def normalize_api_base(api_base):
    ...
    # Be forgiving when users paste Markdown-formatted URLs such as
    # `https://models.sjtu.edu.cn/api/v1` or strings with accidental spaces.
...
def load_api_key(args):
    ...
    for env_name in ("DEEPSEEK_API_KEY", "SJTU_API_KEY"):
        key = normalize_api_key(os.environ.get(env_name, ""))
        if key:
            return key, env_name
    raise SystemExit(
        "Missing API key. Set DEEPSEEK_API_KEY or SJTU_API_KEY, pass --api-key, "
...
def deepseek_chat(api_key, api_base, model, messages, timeout, temperature, max_tokens):
    ...
    for attempt in range(1, args.max_retries + 1):
        try:
            ...
            if exc.code in (401, 403):
                raise SystemExit(
                    f"API returned {exc.code}. Check that the key is valid for "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Step-Audio2 responses with DeepSeek/SJTU API."
    )
...
def main():
    ...
    if args.auth_check_only:
        try:
            ...
            raise SystemExit(
                "auth_check=failed network_error=%s. The API key was loaded, "
                "but this machine could not connect to the API endpoint. "
                "Try a longer --timeout, verify the endpoint with curl, or run "
                "from a network environment that can access models.sjtu.edu.cn."
                % exc
            ) from exc
